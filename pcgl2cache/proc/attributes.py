import numpy as np
from ..client.serializers import NumPyArray
from ..client.serializers import NumPyValue
from ..client.bigtable.attributes import Attribute

UINT64 = np.dtype("uint64").newbyteorder("L")
UINT32 = np.dtype("uint32").newbyteorder("L")
UINT16 = np.dtype("uint16").newbyteorder("L")
FLOAT16 = np.dtype("uint64").newbyteorder("L")
...
REP_COORD_NM = Attribute(
    key=b"rep_coord_nm",
    family_id="0",
    serializer=NumPyArray(dtype=UINT64, compression_level=17),
)

CHUNK_INTERSECT_COUNT = Attribute(
    key=b"chunk_intersect_count",
    family_id="0",
    serializer=NumPyArray(dtype=UINT16, shape=(-1, 3), compression_level=17),
)

PCA = Attribute(
    key=b"rep_coord_nm",
    family_id="0",
    serializer=NumPyArray(dtype=FLOAT16, shape=(-1, 3), compression_level=17),
)
